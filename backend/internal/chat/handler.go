...
// GenerateWSToken issues a short-lived, one-time token used for WebSocket handshake.
// POST /api/v1/chat/ws-token (auth required)
func (h *Handler) GenerateWSToken(c *gin.Context) {
	userID := c.MustGet("user_id").(string)
	wsToken := uuid.NewString()
	if err := h.rdb.Set(context.Background(), wsTokenPrefix+wsToken, userID, wsTokenTTL).Err(); err != nil {
		response.InternalError(c, err)
		return
	}
	response.OK(c, gin.H{"ws_token": wsToken, "expires_in_seconds": int(wsTokenTTL.Seconds())})
}
...
func (h *Handler) SendMessage(c *gin.Context) {
	...
	go func() {
		...
		h.db.Select("name").First(&sender, "id = ?", userID)
		senderName := sender.Name
		if senderName == "" {
			senderName = "Someone"
		}

		var recipients []ConversationMember
		h.db.Where("conversation_id = ? AND user_id != ?", convID, userID).Find(&recipients)
		for _, r := range recipients {
			notifyNewMessage(r.UserID, senderName, req.Content, convID.String())
		}
	}()
