...
func ServeWS(hub *Hub, c *gin.Context, db *gorm.DB) {
	// Authenticate via one-time WebSocket token minted from POST /api/v1/chat/ws-token.
	wsToken := c.Query("token")
	if wsToken == "" {
		c.JSON(http.StatusUnauthorized, gin.H{"error": "missing ws token query parameter"})
		return
	}
	userID, err := consumeWSToken(hub.rdb, wsToken)
	if err != nil {
		c.JSON(http.StatusUnauthorized, gin.H{"error": "invalid or expired ws token"})
		return
	}
...
func consumeWSToken(rdb *redis.Client, wsToken string) (string, error) {
	return rdb.GetDel(context.Background(), wsTokenPrefix+wsToken).Result()
}
