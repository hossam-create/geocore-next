package images
  ...
  func ValidateImageFile(fh *multipart.FileHeader) error {
  	...
  	// Detect MIME from declared Content-Type or file extension
  	...
  	if contentType == "" {
  		...
  		default:
  			return fmt.Errorf("file %q: unsupported extension", fh.Filename)
  		}
  	}

  	mimeType, _, _ := mime.ParseMediaType(contentType)
  	if allowed, known := allowedMIME[strings.ToLower(mimeType)]; !known || !allowed {
  		return fmt.Errorf("file %q: unsupported type %q (accepted: JPEG, PNG, GIF)",
