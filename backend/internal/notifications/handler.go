...
func (h *Handler) RegisterPushToken(c *gin.Context) {
	...
	// Delegate to the production PushService device registry
	dev, err := push.Default().RegisterDevice(c.Request.Context(), userUUID, req.Token, req.Platform, req.AppVersion)
	if err != nil {
		slog.Error("failed to register push token", "user_id", userID, "error", err.Error())
		response.InternalError(c, err)
		return
	}

	response.Created(c, gin.H{"id": dev.ID, "platform": dev.Platform, "is_active": dev.IsActive, "message": "Push token registered."})
}
...
func (h *Handler) DeletePushToken(c *gin.Context) {
	...
	// Delegate to the production PushService device registry
	if err := push.Default().UnregisterDevice(c.Request.Context(), parsedID); err != nil {
		slog.Error("failed to unregister push token", "device_id", parsedID, "error", err.Error())
		response.InternalError(c, err)
		return
	}
