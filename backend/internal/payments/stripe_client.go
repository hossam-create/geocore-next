...
func InitStripe() {
	key := ops.ConfigGet("STRIPE_SECRET_KEY")
	if key == "" {
...
func createStripeCustomer(email, name, phone string) (string, error) {
	...
	err := retry.DoWithContextAndJitter(context.Background(), 3, 2*time.Second, func() error {
		return circuit.PaymentsBreaker.Execute(func(ctx context.Context) error {
...
func VerifyWebhookSignature(payload []byte, sigHeader string) (*stripe.Event, error) {
	secret := ops.ConfigGet("STRIPE_WEBHOOK_SECRET")
	if secret == "" {
