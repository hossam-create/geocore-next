...
type RateLimiter struct {
	...
	// OnReject is an optional callback invoked when a request is rejected
	// due to rate limiting. It allows the application layer to perform side
	// effects like security audit logging without adding dependencies here.
	OnReject func(c *gin.Context, key, path string, limit int, window time.Duration, retryAfter int64)
}
...
func (rl *RateLimiter) Limit(limit int, window time.Duration, prefix string) gin.HandlerFunc {
	return rl.limitWith(limit, window, func(c *gin.Context) string {
		return fmt.Sprintf("ratelimit:%s:%s", prefix, normalizeClientIP(c.ClientIP()))
	})
}
...
func (rl *RateLimiter) limitWith(
	...
	return func(c *gin.Context) {
		...
		if rl.whitelist[normalizeClientIP(c.ClientIP())] {
			c.Next()
			return
		}
		...
		if remaining == 0 && current >= limit {
			...
			if rl.OnReject != nil {
				rl.OnReject(c, key, path, limit, window, retryAfter)
			}
...
func normalizeClientIP(ip string) string {
	...
	if parsed.IsLoopback() {
		...
		// Normalize IPv6 loopback (::1) to IPv4 loopback for stable keys.
		return "127.0.0.1"
	}
