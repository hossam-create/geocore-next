package email
  ...
  // send transmits a plain-text email.  If SMTP is unconfigured it falls back
  // to stdout logging so development works without a mail server.
  func send(cfg smtpConfig, to, subject, body string) error {
  	...
  	return smtp.SendMail(addr, auth, cfg.From, []string{to}, []byte(sb.String()))
  }
