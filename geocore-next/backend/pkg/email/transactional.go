...
// SendOrderReceivedSellerEmail notifies a seller that a new order was placed for their item.
func SendOrderReceivedSellerEmail(to, name, itemTitle string, amount float64, currency string) error {
        cfg := loadSMTP()
        if name == "" {
                name = "there"
        }
        ...
        body := fmt.Sprintf(
                "Hi %s,\n\n"+
                        ...
                        "  Item: %s\n"+
                        "  Amount: %.2f %s (held in escrow)\n\n"+
                        "The buyer's payment is held in escrow until you fulfil the order.\n"+
                        "Manage your orders at:\n"+
                        "  %s/dashboard\n\n"+
                        "Need help? Contact support@geocore.app\n\n"+
                        "— The GeoCore Team",
                name, itemTitle, amount, currency, cfg.BaseURL,
        ...
        if cfg.Host == "" || cfg.From == "" {
                fmt.Printf("[email-dev] New order email sent to seller %s for '%s'\n", to, itemTitle)
                return nil
        }
        return send(cfg, to, fmt.Sprintf("New order: %s", itemTitle), body)
}

// SendEscrowReleasedEmail notifies a seller that escrow funds have been released to them.
func SendEscrowReleasedEmail(to, name string, amount float64, currency string) error {
        cfg := loadSMTP()
        if name == "" {
                name = "there"
        }
        ...
        body := fmt.Sprintf(
                "Hi %s,\n\n"+
                        ...
                        "  Amount Released: %.2f %s\n\n"+
                        "The funds will appear in your account within 1-3 business days.\n"+
                        "View your earnings at:\n"+
                        "  %s/dashboard\n\n"+
                        "— The GeoCore Team",
                name, amount, currency, cfg.BaseURL,
        )

        if cfg.Host == "" || cfg.From == "" {
                fmt.Printf("[email-dev] Escrow release email sent to seller %s: %.2f %s\n", to, amount, currency)
                return nil
        }
        return send(cfg, to, fmt.Sprintf("%.2f %s released from escrow", amount, currency), body)
}
