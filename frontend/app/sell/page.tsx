...
export default function SellPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (isAuction) {
        ...
        router.push(newId ? `/auctions/${newId}` : "/auctions");
      } else {
        const payload: Record<string, unknown> = {
          title: form.title,
          description: form.description,
        ...
        router.push(newId ? `/listings/${newId}` : "/listings");
      }
    } catch (err: any) {
      const msg =
  ...
  const steps = [t("category"), t("description"), t("condition")];
      ...
      <div className="max-w-2xl mx-auto">
            ...
            <>
              <div>
                <p className="text-sm font-semibold text-gray-700 mb-3">
                  {t("fixedPrice")} <span className="text-red-500">*</span>
                </p>
        ...
        <div className="flex items-center justify-between mt-6">
          ...
          {step < 3 ? (
            ...
            >
              {isSubmitting ? "Posting…" : t("publish")}
            </button>
          )}
        </div>
      </div>
