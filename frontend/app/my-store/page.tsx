...
interface StorefrontAnalytics {
  ...
  top_listings: { id: string; title: string; views: number; orders: number; conversion_rate: number }[];
...
export default function MyStorefrontPage() {
  ...
  const { data: analyticsData, isLoading: analyticsLoading, error: analyticsError } = useQuery<StorefrontAnalytics>({
    queryKey: ['storefront-analytics', analyticsPeriod],
    queryFn: () => api.get(`/analytics/storefront?period=${analyticsPeriod}`).then(r => r.data),
    enabled: isAuthenticated && hasStore && activeTab === 'analytics',
  });

  const daysMap: Record<string, number> = { '7d': 7, '30d': 30, '90d': 90 };
  const { data: dailyViews } = useQuery<{ date: string; total: number; unique: number }[]>({
    queryKey: ['my-views-summary', analyticsPeriod],
    queryFn: () => api.get(`/listings/views/summary?days=${daysMap[analyticsPeriod] || 30}`).then(r => r.data?.data ?? r.data),
        ...
        <div className="mb-8">
          <div className="bg-white rounded-2xl shadow-sm overflow-hidden">
            ...
            <div className="px-6 pb-6">
              ...
              <p className="text-xs text-gray-400 mt-0.5">mnbarh.com/stores/{storefront.slug}</p>
        ...
        <div className="bg-white rounded-2xl shadow-sm overflow-hidden">
          <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
            <h2 className="text-lg font-bold text-gray-900 flex items-center gap-2">
              <BarChart3 size={20} className="text-[#0071CE]" /> Storefront Analytics
            </h2>
            <div className="flex items-center gap-2">
              ...
              <select
                ...
                className="text-sm border border-gray-200 rounded-lg px-3 py-1.5 focus:outline-none focus:ring-1 focus:ring-[#0071CE]"
              >
          ...
          {analyticsLoading ? (
            <div className="p-12 flex items-center justify-center">
              <Loader2 size={24} className="animate-spin text-[#0071CE]" />
            </div>
          ) : analyticsError ? (
            ...
            <div className="p-6">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                <div className="bg-gray-50 rounded-xl p-4">
                  ...
                  <p className="text-xl font-bold text-gray-900">{analyticsData.total_views.toLocaleString()}</p>
                </div>
                <div className="bg-gray-50 rounded-xl p-4">
                  <div className="flex items-center gap-2 text-gray-400 mb-1">
                    <Users size={14} />
                  ...
                  <p className="text-xl font-bold text-gray-900">{analyticsData.storefront.follower_count.toLocaleString()}</p>
                </div>
                <div className="bg-gray-50 rounded-xl p-4">
                  <div className="flex items-center gap-2 text-gray-400 mb-1">
                    <Package size={14} />
                  ...
                  <p className="text-xl font-bold text-gray-900">{analyticsData.total_orders.toLocaleString()}</p>
                </div>
                <div className="bg-gray-50 rounded-xl p-4">
                  <div className="flex items-center gap-2 text-gray-400 mb-1">
                    <TrendingUp size={14} />
                  ...
                  <p className="text-xl font-bold text-gray-900">{analyticsData.conversion_rate.toFixed(1)}%</p>
                </div>
              </div>
              ...
              {dailyViews && dailyViews.length > 0 && (
                <div className="mb-6">
                  <h3 className="text-sm font-semibold text-gray-700 mb-3">Daily Views</h3>
                  <div className="h-52">
                    <ResponsiveContainer width="100%" height="100%">
                      <BarChart data={dailyViews}>
                        ...
                        <Tooltip />
                        <Bar dataKey="total" fill="#0071CE" radius={[4, 4, 0, 0]} name="Total Views" />
                        <Bar dataKey="unique" fill="#93c5fd" radius={[4, 4, 0, 0]} name="Unique Views" />
                      </BarChart>
                    </ResponsiveContainer>
                  </div>
                </div>
              ...
              {/* Top Listings */}
              {analyticsData.top_listings.length > 0 && (
                <div>
                  <h3 className="text-sm font-semibold text-gray-700 mb-3">Top Performing Listings</h3>
                  <div className="space-y-2">
                    {analyticsData.top_listings.map((listing, i) => (
                      <div key={listing.id} className="flex items-center gap-3 bg-gray-50 rounded-lg px-4 py-3">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-xs text-gray-400">{listing.views} views · {listing.orders} orders</p>
                        </div>
                        <div className="text-right">
                          <p className="text-sm font-semibold text-[#0071CE]">{listing.conversion_rate.toFixed(1)}%</p>
      ...
      {activeTab === 'listings' && (
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
          ...
          <div className="flex items-center gap-2">
            ...
            <label className="flex items-center gap-1.5 text-xs border border-gray-200 text-gray-600 px-3 py-1.5 rounded-lg hover:bg-gray-50 transition-colors cursor-pointer">
              Import CSV
              <input
                type="file"
                accept=".csv"
                ...
                onChange={async (e) => {
                  ...
                  fd.append("file", file);
                  try {
                    const { data } = await api.post("/listings/import", fd);
                    const r = data.data as { success: number; failed: number };
                    alert(`Imported ${r.success} listings, ${r.failed} failed.`);
                    qc.invalidateQueries({ queryKey: ["my-listings"] });
                  } catch (err: any) {
