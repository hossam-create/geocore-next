...
import {
  addWatchlistItem,
  removeWatchlistItem,
  removeWatchlistPriceSnapshot,
  setWatchlistPriceSnapshot,
} from "@/lib/api";
...
interface ListingCardProps {
  ...
  onFavoriteChange?: (id: string, watched: boolean) => void;
...
export function ListingCard({ listing, onFavorite, onFavoriteChange, forceWatched }: ListingCardProps) {
  const router = useRouter();
  const pathname = usePathname();
  const qc = useQueryClient();
  const { isAuthenticated } = useAuthStore();

  const defaultWatched = forceWatched ?? listing.is_watched ?? listing.isWatched ?? false;
  const [saved, setSaved] = useState(defaultWatched);
  const [added, setAdded] = useState(false);

  useEffect(() => {
    setSaved(forceWatched ?? listing.is_watched ?? listing.isWatched ?? false);
  }, [forceWatched, listing.id, listing.is_watched, listing.isWatched]);
  ...
  const location: string = listing.city || listing.location?.city || (typeof listing.location === 'string' ? listing.location : '') || "GCC";
  ...
  const watchlistMutation = useMutation({
    mutationFn: async (nextState: boolean) => {
      if (nextState) {
        await addWatchlistItem(listing.id);
        if (typeof price === "number" && price > 0) {
          setWatchlistPriceSnapshot(listing.id, price);
        }
      } else {
        await removeWatchlistItem(listing.id);
        removeWatchlistPriceSnapshot(listing.id);
      }
      return nextState;
    },
    onSuccess: (nextState) => {
  ...
  const handleSave = (e: React.MouseEvent) => {
    ...
    if (!isAuthenticated) {
      router.push(`/login?next=${encodeURIComponent(pathname || "/listings")}`);
      return;
    }

    if (watchlistMutation.isPending) return;
    watchlistMutation.mutate(!saved);
  };
      ...
      <div className="bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-xl border border-gray-100 hover:border-[#0071CE]/20 transition-all duration-200 hover:-translate-y-0.5 cursor-pointer group h-full flex flex-col">
        ...
        <div className="relative bg-gray-50 overflow-hidden" style={{ aspectRatio: "4/3" }}>
          ...
          <button
            ...
            className={`absolute top-2.5 right-2.5 w-8 h-8 rounded-full flex items-center justify-center transition-all shadow-sm disabled:opacity-60 ${saved ? "bg-red-500 text-white" : "bg-white/90 text-gray-400 hover:text-red-500 hover:bg-white"}`}
          >
        ...
        <div className="p-3 flex flex-col flex-1 gap-1.5">
          ...
          {/* Sprint 9: Urgency Badge */}
          {FeatureFlags.urgencyBadges && (
            <UrgencyBadge
              watchersCount={listing.watchers_count}
              viewsToday={listing.views_today}
              endsAt={endsAt}
              isAuction={isAuction}
            />
          )}
