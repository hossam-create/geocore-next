...
export function useListingActions() {
  ...
  const approve = useMutation({
    mutationFn: (id: string) => api.post(`/admin/listings/${id}/approve`).catch(() => true),
    onSuccess: invalidate
  });

  const reject = useMutation({
    mutationFn: (id: string) => api.post(`/admin/listings/${id}/reject`).catch(() => true),
    onSuccess: invalidate
  });

  const bulkApprove = useMutation({
    mutationFn: (ids: string[]) => api.post(`/admin/listings/bulk-approve`, { ids }).catch(() => true),
    onSuccess: invalidate
  });

  const bulkReject = useMutation({
    mutationFn: (ids: string[]) => api.post(`/admin/listings/bulk-reject`, { ids }).catch(() => true),
    onSuccess: invalidate
  });
