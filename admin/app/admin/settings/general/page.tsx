...
export default function AdminGeneralSettingsPage() {
  ...
  const { data: settings = [], isLoading } = useQuery<AdminSetting[]>({
    ...
    queryFn: () => settingsApi.getByCategory("site"),
  });
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-slate-500 mt-0.5">Site name, logo, maintenance mode, and defaults</p>
      </div>
      <div className="bg-white rounded-xl border border-slate-200 p-5">
        {isLoading ? (
          <p className="text-sm text-slate-400 py-8 text-center">Loading settings...</p>
        ) : (
