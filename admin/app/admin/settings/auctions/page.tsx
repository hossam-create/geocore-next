...
export default function AdminAuctionsSettingsPage() {
  const qc = useQueryClient();
  const showToast = useToastStore((s) => s.showToast);
  const { data: settings = [], isLoading } = useQuery<AdminSetting[]>({
    queryKey: ["admin-settings", "auctions"],
    queryFn: () => settingsApi.getByCategory("auctions"),
  });
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-slate-900">Auctions Settings</h1>
        <p className="text-sm text-slate-500 mt-0.5">Proxy bidding, bid increments, anti-snipe extensions</p>
      </div>
